...
const LoginPage = ({ onLogin, attemptLogin, onRegister }: { onLogin: (role: UserRole) => void, attemptLogin: (email: string, password: string, role: UserRole) => Promise<boolean>, onRegister: (name: string, email: string, password: string, role: UserRole) => Promise<boolean> }) => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 flex flex-col items-center justify-center p-6 relative overflow-hidden text-zinc-900 dark:text-white">
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in duration-500 mt-8">
        <div className="flex flex-col items-center mb-6">
          ...
          <h1 className="text-5xl font-black tracking-tighter text-zinc-900 dark:text-white">
            ...
            <span className="text-zinc-400 dark:text-white/80">Presyo</span>
          </h1>
          ...
          {showRegister ? (
            <form onSubmit={handleRegister} className="space-y-6">
              ...
              <div className="flex gap-4">
                <button type="button" onClick={() => setShowRegister(false)} className="flex-1 bg-zinc-50 dark:bg-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-400 py-3 rounded-2xl font-black uppercase tracking-widest border border-zinc-200 dark:border-zinc-700">Back to Login</button>
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-white flex flex-col selection:bg-green-400/30 animate-in fade-in duration-1000">
      <Ticker crops={crops} onCropClick={(crop) => setSelectedCrop(crop)} />

      <header className="sticky top-10 z-40 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-2xl border-b border-zinc-200 dark:border-zinc-800 px-3 sm:px-8 py-3 sm:py-5">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {role === UserRole.VENDOR && (
              <div className="relative">
                <button
                  ...
                  className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center text-zinc-500 hover:text-yellow-400 transition-all hover:border-yellow-400/30 shadow-xl relative"
                >
                  <Bell size={22} />
                ...
                {showNotifications && (
                  <div className="absolute right-0 top-16 w-80 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl overflow-hidden z-50">
            ...
            <button onClick={handleLogout} className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center text-zinc-500 hover:text-red-500 transition-all hover:border-red-500/30 shadow-xl group"><LogOut size={20} className="group-hover:-translate-x-1 transition-transform" /></button>
          </div>
        </div>
      </header>
      ...
      {/* Footer */}
      <footer className="bg-white dark:bg-zinc-950 border-t border-zinc-200 dark:border-zinc-800 mt-20 hidden lg:block">
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-2xl border-t border-zinc-200 dark:border-zinc-800 px-4 sm:px-8 py-3 sm:py-5 flex justify-around items-center z-50 rounded-t-3xl sm:rounded-t-[40px] shadow-[0_-20px_50px_rgba(0,0,0,0.1)]">
