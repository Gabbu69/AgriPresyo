...
const formatPrice = (price: number, currency: Currency) => {
  const value = currency === Currency.USD ? price / USD_PHP_RATE : price;
  const symbol = currency === Currency.USD ? '$' : '₱';
  return `${symbol}${value.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
};

const Ticker = ({ crops, currency }: { crops: Crop[], currency: Currency }) => {
  return (
      ...
      <div className="animate-marquee flex gap-8 px-4">
        {[...crops, ...crops].map((crop, idx) => (
          <div key={`${crop.id}-${idx}`} className="flex items-center gap-2 font-mono text-sm">
            ...
            <span className="font-bold text-white">{formatPrice(crop.currentPrice, currency)}</span>
...
const Sparkline = ({ data, color }: { data: any[], color: string }) => (
  <div className="h-12 w-24">
    <ResponsiveContainer width="100%" height="100%">
      <AreaChart data={data}>
        <defs>
          <linearGradient id={`gradient-${color}`} x1="0" x2="0" y2="1">
            <stop offset="5%" stopColor={color} stopOpacity={0.3}/>
            <stop offset="95%" stopColor={color} stopOpacity={0}/>
          </linearGradient>
        </defs>
        <Area type="monotone" dataKey="price" stroke={color} fill={`url(#gradient-${color})`} strokeWidth={2} dot={false} />
      </AreaChart>
    </ResponsiveContainer>
  </div>
);
    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in duration-500">
        <div className="flex flex-col items-center mb-10">
          <Logo size={96} className="text-green-500 mb-6 drop-shadow-[0_0_20px_rgba(34,197,94,0.3)]" />
          <h1 className="text-5xl font-black tracking-tighter text-white">
            <span className="text-green-600">Agri</span>
            <span className="text-zinc-300">Presyo</span>
          </h1>
          <p className="text-zinc-500 font-bold mt-2 uppercase tracking-widest text-[10px]">Market Intelligence Terminal</p>
        </div>

        <div className="bg-zinc-900/50 border border-zinc-800 rounded-[40px] p-8 backdrop-blur-xl shadow-2xl">
          <div className="flex bg-zinc-950 p-1.5 rounded-2xl border border-zinc-800 mb-8">
            <button 
...
const App = () => {
  ...
  const [role, setRole] = useState<UserRole>(UserRole.CONSUMER);
  const [currency, setCurrency] = useState<Currency>(Currency.PHP);
  ...
  const filteredVendors = useMemo(() => {
    return allVendors.filter(v => {
      ...
      if (shopFilter === 'Vegetable') return v.cropsSold.some((c: any) => c.category !== 'Fruit');
      return true;
    });
  ...
  const filteredCrops = useMemo(() => {
    return crops.filter(c => {
      ...
      const matchesCat = activeCategory === 'All' || c.category === activeCategory || (activeCategory === 'Organic' && c.demand === 'High');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {['All', 'Fruit', 'Vegetable', 'Spice', 'Root', 'Organic'].map(cat => (
              <button 
                key={cat} 
              ...
              <div className="flex justify-between items-start relative z-10">
                ...
                <div className="text-right">
                  <p className="font-mono text-xl font-bold text-white">
                    {formatPrice(crop.currentPrice, currency)}
                  </p>
    ...
    <div className="bg-zinc-950 border border-zinc-800 rounded-[40px] p-10 shadow-2xl relative overflow-hidden mt-12 mb-20 border-t-green-400/30">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-10">
        ...
        <div className="flex items-center gap-4 bg-zinc-900 p-4 rounded-3xl border border-zinc-800 shadow-inner">
          ...
          />
          <span className="text-sm font-black text-zinc-600 font-mono">{currency}</span>
        </div>
      </div>
                ...
                <div key={item.cropId} className="flex items-center justify-between bg-zinc-900/50 hover:bg-zinc-900 p-5 rounded-3xl border border-zinc-800 transition-colors group">
                  ...
                  <div className="flex items-center gap-8">
                    ...
                    <div className="text-right w-32">
                      <p className="font-mono text-2xl font-bold text-white">{formatPrice(weight * crop.currentPrice, currency)}</p>
                    </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-10">
              <div className="text-center md:text-left flex-1">
                ...
                <div className="flex items-baseline gap-4">
                  ...
                  <span className="text-6xl font-black font-mono text-white tracking-tighter">{formatPrice(budgetStats.totalCost, currency)}</span>
                </div>
              </div>
    ...
    <div className="space-y-12 pb-32 lg:pb-12 animate-in slide-in-from-bottom duration-700">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-zinc-900 p-8 rounded-[40px] border border-zinc-800 relative overflow-hidden group shadow-2xl flex flex-col justify-center">
          ...
          <div className="text-5xl font-black text-white tracking-tighter">BULLISH</div>
          <p className="text-green-400 font-bold text-xs tracking-widest uppercase mt-2">Optimal Liquidity Period</p>
        </div>
                    ...
                    <div className="grid grid-cols-2 gap-6 flex-1 lg:max-w-xl">
                      <div className="bg-zinc-950 p-6 rounded-3xl border border-zinc-800 shadow-inner group/stat">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-2xl font-mono font-bold text-green-400 leading-none">{formatPrice(myEntry.price, currency)}</span>
  ...
  return (
    <div className="min-h-screen flex flex-col selection:bg-green-400/30 animate-in fade-in duration-1000">
      <Ticker crops={crops} currency={currency} />
        ...
        <div className="max-w-[1400px] mx-auto flex items-center justify-between">
          <div className="flex items-center gap-12">
            <div className="flex items-center gap-4 group cursor-pointer" onClick={() => setActiveTab('market')}>
              <Logo size={48} className="text-green-500 transition-all transform group-hover:scale-110 group-hover:rotate-6" />
          ...
          <div className="flex items-center gap-4">
            <div className="flex bg-zinc-900 rounded-xl p-1.5 border border-zinc-800 shadow-inner">
              {[Currency.PHP, Currency.USD].map(c => <button key={c} onClick={() => setCurrency(c)} className={`px-6 py-2 rounded-xl text-xs font-black transition-all uppercase tracking-widest ${currency === c ? 'bg-zinc-800 text-white shadow-xl border border-zinc-700' : 'text-zinc-500 hover:text-zinc-300'}`}>{c}</button>)}
            </div>
                ...
                <div className="grid gap-4">
                  {selectedVendor.cropsSold.map((crop: any) => (
                    <div key={crop.id} className="bg-zinc-950 p-6 rounded-[32px] border border-zinc-800 flex items-center justify-between group hover:border-zinc-600 transition-all shadow-lg">
                      ...
                      <div className="text-right">
                        <p className="text-3xl font-black font-mono text-green-500 leading-none mb-2">{formatPrice(crop.vendors.find((v: any) => v.id === selectedVendor.id)?.price || crop.currentPrice, currency)}</p>
          ...
          <div className="bg-zinc-900 w-full max-w-xl rounded-[50px] overflow-hidden relative border border-zinc-800 shadow-2xl animate-in zoom-in-95 duration-200">
             <div className="p-12 space-y-10">
                ...
                <div className="bg-zinc-950 p-8 rounded-[36px] border border-zinc-800 shadow-inner">
                  ...
                  <p className="text-5xl font-black font-mono text-white tracking-tighter">{formatPrice(selectedCrop.currentPrice, currency)} <span className="text-xl text-zinc-600">/ kg</span></p>
                ...
                <div className="space-y-4">
                   ...
                   {selectedCrop.vendors.sort((a,b) => a.price - b.price).map(v => (
                     <div key={v.id} className="flex justify-between items-center bg-zinc-900 border border-zinc-800 p-5 rounded-3xl hover:border-green-400/30 transition-colors shadow-lg group">
                        ...
                        <p className="font-mono font-black text-2xl text-green-500">{formatPrice(v.price, currency)}</p>
                     </div>
                   ))}
                </div>
      ...
      {isAddCropModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          <div className="absolute inset-0 bg-black/98 backdrop-blur-xl" onClick={() => setIsAddCropModalOpen(false)}></div>
              ...
              <div className="space-y-8">
                 ...
                 <div className="grid grid-cols-2 gap-6">
                   ...
                   <div className="space-y-2">
                      ...
                      <input id="admin-s" type="number" placeholder="0" className="w-full bg-zinc-950 border border-zinc-800 rounded-2xl p-5 font-mono text-xl font-bold outline-none text-white focus:border-green-400/50 shadow-inner" />
                   </div>
                 </div>
              </div>
              <button onClick={() => {
