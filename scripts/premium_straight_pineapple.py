...
def make_it_premium():
    ...
    try:
        with Image.open(src) as img:
            ...
            # Premium format: target fruit size = 460 (allows 26px padding on all sides of a 512 canvas)
            target_h = 460
            scale = float(target_h) / max(w, h)
            new_w, new_h = int(w * scale), int(h * scale)
            print(f"Scaling carefully to {new_w}x{new_h} to leave breathing room...")
            fruit = fruit.resize((new_w, new_h), Image.Resampling.LANCZOS)
            
            canvas = Image.new("RGBA", (512, 512), (0, 0, 0, 0))
            offset_x = (512 - new_w) // 2
            
            # If it's very tall, maybe shift it slightly down so leaves aren't crowding the top?
            # A centered 460px height in 512px canvas gives 26px top padding.
            offset_y = (512 - new_h) // 2 
            
            canvas.paste(fruit, (offset_x, offset_y), fruit)
            canvas.save(dst, "PNG")
            
            print("Successfully formatted the pineapple with premium padding and smooth edges!")
