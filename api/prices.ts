export default function handler(req, res) {
  ...
  const FRUIT_VENDORS = [
    {
      ...
      specialty: "South Cotabato",
      rating: 4.8,
      reviewCount: 110,
    },
    {
      id: "v_fruit_2",
      name: "Tupi Tropical Fruits",
      specialty: "South Cotabato",
      rating: 4.9,
      reviewCount: 145,
    },
  ...
  // Pick 3 random vendors for a crop based on category
  const getVendors = (basePrice, cropSeed, category) => {
    const vendorPool = category === "Fruit" ? FRUIT_VENDORS : VEG_VENDORS;
    const selected = [];
    const usedVends = new Set();

    for (let i = 0; i < 3; i++) {
      let vIdx = Math.floor(getRnd(cropSeed + i * 10, 0, vendorPool.length));
      while (usedVends.has(vIdx)) {
        vIdx = (vIdx + 1) % vendorPool.length;
      }
      usedVends.add(vIdx);
      const v = vendorPool[vIdx];

      // Vendor price fluctuation
      const vPrice =
        basePrice + basePrice * getRnd(cropSeed + vIdx * 5, -0.06, 0.08);
  ...
  const getCrop = (
    ...
    const currentPrice = Number(
      (basePrice + basePrice * todaysFluctuation).toFixed(2),
    );
    ...
    return {
      ...
      icon,
      weightPerUnit,
      vendors: getVendors(currentPrice, cropSeed, category),
    };
  };
